...
class EitherTest extends FunSpec{
  ...
  describe("Either map a => a+1") {
    describe("applied to Left option") {
      val leftEither = Left("12323") : Either[String, Int];
      it("should be leave Left at it is") {
    ...
    describe("applied to Right(1) option") {
      val emptyOption = Right(1);
      it("should add one to Right(2)") {
  ...
  describe("Either flatMap a => Some(a+1)") {
    describe("applied to Left(\"Test\") option") {
      val leftEither = Left("Test") : Either[String, Int];
      it("should be leave Left as it is") {
        assertResult(leftEither.flatMap(a => Right(a + 1))) {
          Left("Test")
        }
      }
    }
    ...
    describe("applied to Right(1) option") {
      val rightOne = Right(1);
      it("should add one and return Right(2") {
        assertResult(rightOne.flatMap(a => Right(a+1))) {
          Right(2)
        }
      }
    }
